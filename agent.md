...
- 仅当目标阶段状态为 `complete`，且表中归档核验为 `verified` 并链接核验记录时，才允许精简该阶段数据。进入下一阶段不影响已完成阶段的清理资格；运行中、未完成或仍被其他实验使用的文件不得删除。
- 归档核验须确认各实验唯一的原始数值结果、方案（包括 best 和必要对照）、参数、算法版本及可恢复源码、输入/配置/evaluator 指纹、复现命令、汇总及失败信息保留完整，文件索引可用。仅保存最终汇总表不足以通过核验。
...
- 清理前在目标阶段归档目录保存清单，列出将处理文件的工作区内绝对路径、大小、原哈希、处理理由和替代副本（如有），并检查无运行任务或后续实验依赖。删除或移动前验证解析后的路径仍位于目标实验目录内；不以通配递归删除整个阶段目录。
